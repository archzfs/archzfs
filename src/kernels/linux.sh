...
pkgrel="1"
kernel_version="4.9"
kernel_version_max="4.10"
extramodules="extramodules-${kernel_version}-ARCH"

# ZFS Headers (only needs to be built once per kernel version)
headers_pkgrel="2"
...
update_linux_headers_pkgbuilds() {
    # These packages only need to be built once per kernel version
    # pkg_list=("spl-${kernel_name}-headers" "zfs-${kernel_name}-headers")
    pkg_list=("zfs-${kernel_name}-headers")

    headers_pkgver=${zol_version}_${kernel_version}
    headers_pkgrel=${headers_pkgrel}
    headers_makedepends="\"git\""

    spl_headers_depends=""
    spl_headers_pkgname="spl-${kernel_name}-headers"
    spl_headers_pkgbuild_path="packages/${kernel_name}/${spl_headers_pkgname}"
    ...
    zfs_headers_depends="\"spl-${kernel_name}-headers\""
    zfs_headers_pkgname="zfs-${kernel_name}-headers"
    zfs_headers_pkgbuild_path="packages/${kernel_name}/${zfs_headers_pkgname}"
    ...
    linux_depends_max="\"linux<${kernel_version_max}\""
    linux_headers_depends="\"linux-headers>=${kernel_version}\""
    linux_headers_depends_max="\"linux-headers<${kernel_version_max}\""
